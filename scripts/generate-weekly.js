...
body {
  ...
  line-height: 1.7;
  min-height: 100vh;
  background-image: radial-gradient(ellipse at top, rgba(59, 130, 246, 0.08) 0%, transparent 50%), radial-gradient(ellipse at bottom right, rgba(139, 92, 246, 0.05) 0%, transparent 50%) !important;
...
- 主导语言：${aggregated.topLanguages.map(l => l.name).join(', ')}
...
请输出 JSON 格式的周度洞察：
{
  "weeklyTheme": "一句话总结本周主题（15-25 字，要有洞察力）",
  "hot": [
    "热点洞察 1（结合具体项目和技术趋势，50 字左右）",
    "热点洞察 2",
    "热点洞察 3",
    "热点洞察 4"
  ],
  "trends": [
    "短期趋势 1（1-3 个月，结合技术发展和市场动态）",
    "短期趋势 2",
    "短期趋势 3"
  ],
  "actions": [
    "行动建议 1（具体可执行，针对后端架构师/技术负责人）",
    "行动建议 2",
    "行动建议 3",
    "行动建议 4"
  ],
...
要求：
1. 用中文，要有周度视角的深度洞察，不要泛泛而谈
2. 热点要结合具体项目和技术方向
3. 趋势要体现技术演进方向
4. 行动建议要具体可执行，针对技术决策者
  ...
  try {
    ...
    const result = execSync(`node -e "
      ...
      const req = https.request({
        ...
        res.on('end', () => {
          try {
            ...
            let json = c.trim().replace(/^\\\`\\\`\\\`json\\s*/, '').replace(/\\s*\\\`\\\`\\\`$/, '').replace(/^\\\`\\\`\\\`\\s*/, '');
            console.log(json);
          } catch(e) {
      ...
      req.write(JSON.stringify({
        model: 'bailian/qwen3.5-plus',
        messages: [
          { role: 'system', content: '你是专业技术分析师，擅长从 GitHub Trending 数据中洞察技术趋势。输出纯 JSON，不要 markdown，不要额外文本。' },
    ...
    if (jsonStr.startsWith('ERROR:')) throw new Error(jsonStr);
    
    const insights = JSON.parse(jsonStr);
    return insights;
  } catch (e) {
    log(`⚠️  LLM 分析失败：${e.message}，使用增强版默认分析`);
    
    // 增强版默认分析（基于规则生成更丰富的内容）
...
async function main() {
  ...
  const dateRange = getDateRange(weekId);

...
/**
 * MonthlyScraper - 每月趋势抓取器
 * 继承 GitHubScraper，专门用于抓取 GitHub 每月趋势数据
 */
class MonthlyScraper extends GitHubScraper {
  /**
   * 构造函数
  ...
  getSchedule() {
    ...
    // 每月 1 号 10:00 执行
    return process.env.MONTHLY_SCHEDULE || '0 0 10 1 * *';
  }

  /**
   * 生成输出文件名
   * 格式：github-trending-monthly-YYYY-MM.json
  ...
  getFileName(date = new Date()) {
    ...
    return `github-trending-monthly${languageSuffix}-${year}-${month}.json`;
  }

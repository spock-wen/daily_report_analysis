# moeru-ai/airi

## 基本信息
- 首次上榜：2026-03-05
- 最近上榜：2026-03-07
- 上榜次数：3
- 领域分类：general
- 语言：TypeScript
- GitHub Stars: 40200（最后更新：2026-05-27）

## 核心功能
- 跨平台（Web/macOS/Windows）+ 实时语音 + VRM/Live2D + Minecraft/Factorio 游戏联动
- 开源 Grok Companion，集成 Live2D/VRM/Minecraft 实时联动
- 首个支持实时语音 + Minecraft/Factorio 游戏内交互的开源 AI VTuber 伴侣
- 融合 Neuro-sama 风格人格容器与 Web/macOS/Windows 全端部署

## 版本历史



### 2026-03-05（日报收录）
**来源**: [日报 2026-03-05](../../reports/daily/github-ai-trending-2026-03-05.html)
**分析**: moeru-ai/airi：跨平台（Web/macOS/Windows）+实时语音+VRM/Live2D+Minecraft/Factorio游戏联动的Grok Companion，定义下一代AI数字生命交互范式 - 立即试用shannon对自有Web应用执行无提示渗透测试（docker run -it --rm -v $(pwd):/app keygraphhq/shannon scan http://localhost:3000）


### 2026-03-06（日报收录）
**来源**: [日报 2026-03-06](../../reports/daily/github-ai-trending-2026-03-06.html)
**分析**: moeru-ai/airi：开源Grok Companion，集成Live2D/VRM/Minecraft实时联动，27.3k★反映AI数字生命体（VTuber+Agent+Voice）消费级爆发 - 立即试用microsoft/mcp-for-beginners中的Python MCP Server示例，用curl调用其/mcp/tools/list端点，理解上下文路由机制


### 2026-03-07（日报收录）
**来源**: [日报 2026-03-07](../../reports/daily/github-ai-trending-2026-03-07.html)
**分析**: moeru-ai/airi：首个支持实时语音+Minecraft/Factorio游戏内交互的开源AI VTuber伴侣，融合Neuro-sama风格人格容器与Web/macOS/Windows全端部署 - 立即试用moeru-ai/airi的Web版并贡献Live2D动作插件（TS+WebGL），参与构建AI人格开源标准


### 2026-05-25（日报收录）
**来源**: [日报 2026-05-25](../../daily/github-ai-trending-2026-05-25.html)
**分析**: ⭐ 明星项目，在开发者社区具有重要影响力
🎤 边缘设备语音处理需求增长，轻量化模型受关注
👥 社区活跃度：高 - 社区活跃，持续有开发者关注和贡献


### 2026-05-27（日报收录）
**来源**: [日报 2026-05-27](../../daily/github-ai-trending-2026-05-27.html)
**分析**: 🔥 moeru-ai/airi：4.0w星，开源自托管AI伴侣容器，打通实时语音、游戏交互与多模态情感计算。
📊 稳定增长，社区认可度良好
⭐ 明星项目，在开发者社区具有重要影响力
🎤 边缘设备语音处理需求增长，轻量化模型受关注
👥 社区活跃度：高 - 社区活跃，持续有开发者关注和贡献

## 跨项目关联

| 项目 | 关联类型 | 说明 |
|------|----------|------|
| jamiepine/voicebox | 同领域 | 实时语音交互 · 语音识别/合成 |
| louis-e/arnis | 关键词匹配 | Minecraft 联动 · 游戏领域 |
| microsoft/VibeVoice | 同领域 | 语音 AI · 实时语音交互 |

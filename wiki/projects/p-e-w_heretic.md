# p-e-w/heretic

## 基本信息
- 首次上榜：2026-03-14
- 最近上榜：2026-03-15
- 上榜次数：2
- 领域分类：general
- 语言：Python
- GitHub Stars: 21995（最后更新：2026-05-27）

## 核心功能
- LLM 审查自动删除工具，探索模型对齐新方向，stars 1.4 万

## 版本历史



### 2026-03-14（日报收录）
**来源**: [日报 2026-03-14](../../reports/daily/github-ai-trending-2026-03-14.html)
**分析**: p-e-w/heretic 实现语言模型全自动审查删除（abliteration），突破安全限制 - 评估 fishaudio/fish-speech 替代现有 TTS 引擎，提升语音交互自然度与成本效益


### 2026-03-15（日报收录）
**来源**: [日报 2026-03-15](../../reports/daily/github-ai-trending-2026-03-15.html)
**分析**: p-e-w/heretic - LLM 审查自动删除工具，探索模型对齐新方向，stars 1.4 万 - 深入研究 666ghj/MiroFish 的群体智能架构，可应用于金融预测或多智能体协作场景


### 2026-05-27（日报收录）
**来源**: [日报 2026-05-27](../../daily/github-ai-trending-2026-05-27.html)
**分析**: 📈 今日增长迅速，值得关注
⭐ 明星项目，在开发者社区具有重要影响力
🧠 大模型生态日趋成熟，微调部署工具需求旺盛
👥 社区活跃度：高 - 社区活跃，持续有开发者关注和贡献

## 跨项目关联

| 项目 | 关联类型 | 说明 |
|------|----------|------|
| anthropics/claude-plugins-official | 引用关系 | 版本历史中提及 BitNet 量化 |
| fishaudio/fish-speech | 引用关系 | 版本历史中提及 TTS 评估 |
| obra/superpowers | 关键词匹配 | LLM · 工作流重构 |
